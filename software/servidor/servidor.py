...
class Servidor:
    ...
    def mainLoop(self, SocketObject):
        ...
        if Control and Control.get("usuario") == self.DefaultUser:
            ...
            if self.checkCredentials(Control) == []:
                self.sendError(ConsSocketCommunication, "Credenciais não reconhecidas, tente novamente.")
            else:
    ...
    def admProcedure(self, socketObject, Control):
        if Control.get("arglist") is not None and Control.get("function") is not None:
            adm_response = MAPPED_QUERIES.get(Control.get("function"))(Control.get("arglist"))
    ...
    def createResponseAdm(self, response_to_transform):
        return json.dumps({"database_response": response_to_transform}).encode("UTF-8")
    ...
    def createResponseUser(self, Control):
        ...
        for lista_soft in MAPPED_QUERIES.get(Control.get("function", None))(None):
            dictionary_to_send[lista_soft[0]]["lista_softwares"].append(lista_soft[1])
